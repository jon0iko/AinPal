...
interface ConSectionsTabProps {
  lawId: any;
  hasChapter: any;
}

const ConSectionsTab: React.FC<ConSectionsTabProps> = ({ lawId, hasChapter }) => {
    const [groupedSections, setGroupedSections] = useState<GroupedConSections>({});
    ...
    const filteredGroupedSections = Object.entries(groupedSections).reduce(
        (acc: GroupedConSections, [chapterKey, chapterData]) => {
    ...
    return (
        ...
        <FlatList
            ...
            <View style={styles.chapter}>
                <Text style={styles.chapterTitle}>{chapterData.chapter_title}</Text>
                {chapterData.sections.map((section) => (
                <TouchableOpacity
                    key={section.section_key}
                    style={styles.section}
                    onPress={() =>
                    router.push(`/law-detail/${lawId}/section/${section.section_key}`)
                    }
                >
                    <Text style={styles.sectionNumber}>{section.section_number}</Text>
